...
import { FlagSpriteOptions, SpritesmithCoordinates, SpritesmithProperties } from './types';

const TEMP_DIRECTORY = path.join(process.cwd(), '..', `tmp`).replace(path.join(process.cwd(), '..'), '');
...
export default async function createFlagSprite(options: FlagSpriteOptions): Promise<void> {
  ...
  if (!mergedOptions.spriteDestination) {
    throw new Error(`spriteDestination not provided`);
  }

  if (!mergedOptions.cssDestination) {
    throw new Error(`cssDestination not provided`);
  }
  ...
  const originalPNGPaths = glob.sync(`${process.cwd()}/dist/png/*.png`);
      ...
      (
        ...
        Object.entries(coordinates).forEach(([pngPath, { x, y, width, height }]: [string, SpritesmithCoordinates]) => {
          ...
          const cssProperties: Dictionary<string> = {
            ['background-position']: `${
              ((x - (mergedOptions.center ? (maxWidth - width) / 2 : 0)) / (properties.width - maxWidth)) * 100
